...
% Ο/οι συγγραφείς
\author{Name, Department, University, City, email \\
Name, Department, University, City, email}
...
%
\begin{center}
\section*{Abstract (up to 300 words)}
Text    
\[\]
\textbf{Keywords} (up to 6):
\end{center}
...
%
%
\subsection{Theorems}\label{sec:theorem}
...
%
%
\subsection{Equations}\label{sec:math}
...
%
%
\subsection{Algorithms} \label{sec:alg}
Ένα παράδειγμα είναι ο Αλγόριθμος \textsc{First} που παρουσιάζεται στο Figure~\ref{alg:first}.
...
Οι πίνακες μπορούν να έχουν μορφή αντίστοιχη με εκείνη που έχει ο Table~\ref{tab:example}. Θυμηθείτε ότι οι πίνακες θα πρέπει να έχουν περιγραφεί μέσα στο κείμενο.
...
\begin{enumerate}
    \item Όταν αναφέρεστε σε κεφάλαιο, ενότητα, σχήμα, πίνακα κ.λπ. και ακολουθείται από αριθμό το γράφετε με κεφαλαίο, π.χ., Chapter 3, Section 3.2, Figure 12. Αν δεν υπάρχει αριθμός το γράφετε με μικρό, π.χ., as shown in the previous section.
    \item Κάνετε πάντα αναφορά στο κείμενο για τα σχήματα, τους πίνακες, κ.λπ. Δεν γράφετε δηλαδή «as shown in the following figure» αλλά «as shown in Figure~\ref{fig:dit}».
\end{enumerate}

\begin{figure}
\centering
\includegraphics[scale=.2]{dit-uop-logo-regular-red}
\caption{The logo of the DIT department}
\label{fig:dit}
\end{figure}
...
%
%
\subsection{Related work}\label{sec:cite}
